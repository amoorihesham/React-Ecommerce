...
const SubCategory = () => {
  ...
  async function getSubCateDetails() {
    const { data } = await axios.get(
      `https://ecommerce.routemisr.com/api/v1/subcategories/${params.id}`
    );
    const mainCateName = await axios.get(
      `https://ecommerce.routemisr.com/api/v1/categories/${data?.data.category}`
    );
    ...
    const filterdProducts = await allProds.filter(
      (product) => product.category.name === mainCateName?.data.data.name
    );
