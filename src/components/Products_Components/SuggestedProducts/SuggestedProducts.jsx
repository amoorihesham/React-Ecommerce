...
const SuggestedProducts = ({ products, product, set }) => {
	...
	function handleSug(product) {
		let filterd = products.filter((prod) => prod.category.name == product.category.name && prod.id != product.id);
			...
			<div className='row'>
				{sugProducts ? (
					sugProducts.slice(0, 5).map((sugProd) => (
						<div className='col' key={sugProd.id} onClick={() => handleSug(sugProd)}>
							<Link to={`/products/${sugProd.id}`}>
								<img src={sugProd.imageCover} className='w-100 mb-3' alt={sugProd.title} />
					...
					<h4 className='text-center'>No Suggested Product In This Category.</h4>
				)}
			</div>
		</div>
	);
