		...
		return (
			<div className='cartPage py-2 h-100vh'>
				<div className='container'>
					<div className='wraper bg-main-light p-4 rounded'>
						<h3>Shop Cart:</h3>
						<p className='text-main'>Total Cart Price: {userCart?.totalPrice || 0} EGP</p>
						<p className='text-main'>Total Cart Items ({userCart?.cartCount || 0})</p>
						<button className='btn bg-main text-white mb-4' onClick={clearCart}>
							Clear Cart
						</button>

						{userCart?.products?.length > 0 ? (
							userCart?.products?.map((product) => (
								<div className='row align-items-center py-2 border-bottom ' key={product?._id}>
									<div className='col-md-8'>
										<div className='box d-flex align-items-center gap-3'>
											<img src={product?.product?.imageCover} alt={product?.product?.title} className='w-25' />
						...
						<Link className='btn bn-sm bg-main text-white mt-3 ms-auto w-50' to={'/checkout'}>
							Procced To checkout
						</Link>
					</div>
				</div>
