...
const Profile = () => {
	const validationSchema = object({
		cPassword: string().min(6, 'Password Minimum Is 6 Chars').max(16, 'Password Maximum Is 16 Chars').required('Please Set Your Password'),
				...
				<div className='topInfo d-flex align-items-center justify-content-between'>
					<div className='left d-flex align-items-center'>
						...
						<div className='info ms-3'>
							...
							<h6 className=' fs-6 text-black-50 m-0 mt-1 '>{userData.email}</h6>
						</div>
					</div>
					<div className='right d-flex gap-3'>
						<button className='btn bg-main text-white' onClick={() => setReqChangePass(true)}>
							Change Password
						</button>
					</div>
				</div>
					...
					<div className='user-password mt-5'>
						<form onSubmit={formik.handleSubmit} className=' w-32'>
							...
							<div className='actions mt-3 d-flex align-items-center justify-content-between'>
								...
								<button className='btn bg-main text-white' onClick={() => setReqChangePass(false)}>
									Cancle
								</button>
							</div>
						</form>
